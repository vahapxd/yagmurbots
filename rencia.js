    ...
    files.forEach(f => {
        readdir("./src/commands/" + f, (err2, files2) => {
            ...
            files2.forEach(file => {
                ...
                console.log(`[Rencia-Supervizor] ${prop.name} yüklendi!`);
...
readdir("./src/events", (err, files) => {
    ...
    files.filter((file) => file.endsWith(".js")).forEach((file) => {
        ...
        console.log(`[Chaos-Supervizor] ${prop.conf.name} yüklendi!`);
    });
});
...
client.on("message", async message => {
    if (message.content === "!buton-rol" && message.author.id === config.bot.owner) {
        ...
        message.channel.send(`Merhaba! BUBBLES'a Hoşgeldiniz
